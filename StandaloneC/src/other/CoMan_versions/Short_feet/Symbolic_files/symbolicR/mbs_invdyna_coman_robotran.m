...
% Forward Kinematics 
  ...
  ALPHA122 = C22*(ALPHA121+BETA221*s.dpt(2,37)+BETA321*s.dpt(3,37)+BS121*s.dpt(1,37))-S22*(ALPHA321+BETA721*s.dpt(1,37)+BETA821*s.dpt(2,37)+BS921...
 *s.dpt(3,37));
  ALPHA222 = ALPHA221+BETA421*s.dpt(1,37)+BETA621*s.dpt(3,37)+BS521*s.dpt(2,37);
  ALPHA322 = C22*(ALPHA321+BETA721*s.dpt(1,37)+BETA821*s.dpt(2,37)+BS921*s.dpt(3,37))+S22*(ALPHA121+BETA221*s.dpt(2,37)+BETA321*s.dpt(3,37)+BS121...
 *s.dpt(1,37));
  OM123 = qd(23)+OM122;
  ...
  ALPHA126 = C26*(ALPHA121+BETA221*s.dpt(2,38)+BETA321*s.dpt(3,38)+BS121*s.dpt(1,38))-S26*(ALPHA321+BETA721*s.dpt(1,38)+BETA821*s.dpt(2,38)+BS921...
 *s.dpt(3,38));
  ALPHA226 = ALPHA221+BETA421*s.dpt(1,38)+BETA621*s.dpt(3,38)+BS521*s.dpt(2,38);
  ALPHA326 = C26*(ALPHA321+BETA721*s.dpt(1,38)+BETA821*s.dpt(2,38)+BS921*s.dpt(3,38))+S26*(ALPHA121+BETA221*s.dpt(2,38)+BETA321*s.dpt(3,38)+BS121...
 *s.dpt(1,38));
  OM127 = qd(27)+OM126;
...
% Backward Dynamics 
 ...
 s.dpt(3,52))+S29*(ALPHA328+BS928*s.dpt(3,52))));
  Fs229 = -(s.frc(2,29)-s.m(29)*(ALPHA228+BETA628*s.dpt(3,52)+s.l(1,29)*(BS229+OMp329)-s.l(2,29)*(OM129*OM129+OM329*OM329)+s.l(3,29)*(BS629-...
 OMp129)));
  Fs329 = -(s.frc(3,29)-s.m(29)*(s.l(1,29)*(BS329-OMp229)+s.l(2,29)*(BS629+OMp129)-s.l(3,29)*(OM129*OM129+OM229*OM229)+C29*(ALPHA328+BS928*...
 s.dpt(3,52))+S29*(ALPHA128+BETA328*s.dpt(3,52))));
 ...
 OM329)+OM229*(s.In(1,29)*OM129+s.In(2,29)*OM229));
  ...
  Fq228 = Fs228+Fs229;
  Cq128 = -(s.trq(1,28)-s.In(1,28)*OMp128-s.In(3,28)*OMp328-Cq129*C29-Cq329*S29+Fs228*s.l(3,28)+Fs229*s.dpt(3,52)-Fs328*s.l(2,28)-OM228*(...
 ...
 s.In(9,28)*OM328)-OM328*(s.In(1,28)*OM128+s.In(3,28)*OM328)-s.dpt(3,52)*(Fs129*C29+Fs329*S29));
 ...
 s.In(3,27)*OM127+s.In(6,27)*OM227+s.In(9,27)*OM327)+OM327*(s.In(2,27)*OM127+s.In(5,27)*OM227+s.In(6,27)*OM327)+s.dpt(3,50)*(Fq128*S28+Fq228*C28));
 ...
 s.In(3,27)*OM127+s.In(6,27)*OM227+s.In(9,27)*OM327)-OM327*(s.In(1,27)*OM127+s.In(2,27)*OM227+s.In(3,27)*OM327)-s.dpt(3,50)*(Fq128*C28-Fq228*S28));
 ...
 s.In(5,27)*OM227+s.In(6,27)*OM327)+OM227*(s.In(1,27)*OM127+s.In(2,27)*OM227+s.In(3,27)*OM327));
  ...
  Cq126 = -(s.trq(1,26)-Cq127-s.In(1,26)*OMp126+Fs226*s.l(3,26)-Fs326*s.l(2,26)+OM226*OM326*(s.In(5,26)-s.In(9,26))-s.dpt(2,48)*(Fq227*S27+Fq327*...
 C27));
  Cq226 = -(s.trq(2,26)-s.In(5,26)*OMp226-Cq227*C27+Cq327*S27-Fs126*s.l(3,26)+Fs326*s.l(1,26)-OM126*OM326*(s.In(1,26)-s.In(9,26)));
  Cq326 = -(s.trq(3,26)-s.In(9,26)*OMp326-Cq227*S27-Cq327*C27+Fq127*s.dpt(2,48)+Fs126*s.l(2,26)-Fs226*s.l(1,26)+OM126*OM226*(s.In(1,26)-...
 s.In(5,26)));
 ...
 s.dpt(3,44))+S25*(ALPHA324+BS924*s.dpt(3,44))));
  Fs225 = -(s.frc(2,25)-s.m(25)*(ALPHA224+BETA624*s.dpt(3,44)+s.l(1,25)*(BS225+OMp325)-s.l(2,25)*(OM125*OM125+OM325*OM325)+s.l(3,25)*(BS625-...
 OMp125)));
  Fs325 = -(s.frc(3,25)-s.m(25)*(s.l(1,25)*(BS325-OMp225)+s.l(2,25)*(BS625+OMp125)-s.l(3,25)*(OM125*OM125+OM225*OM225)+C25*(ALPHA324+BS924*...
 s.dpt(3,44))+S25*(ALPHA124+BETA324*s.dpt(3,44))));
  ...
  Fq224 = Fs224+Fs225;
  Cq124 = -(s.trq(1,24)-s.In(1,24)*OMp124-s.In(2,24)*OMp224-s.In(3,24)*OMp324-Cq125*C25-Cq325*S25+Fs224*s.l(3,24)+Fs225*s.dpt(3,44)-Fs324*...
 ...
 *(s.In(1,24)*OM124+s.In(2,24)*OM224+s.In(3,24)*OM324)-s.dpt(3,44)*(Fs125*C25+Fs325*S25));
 ...
 s.In(5,23)*OM323+s.In(9,23)*OM323)+s.dpt(3,42)*(Fq124*S24+Fq224*C24));
 ...
 s.In(1,23)*OM123+s.In(3,23)*OM323)-s.dpt(3,42)*(Fq124*C24-Fq224*S24));
 ...
 *(s.In(2,22)*OM122+s.In(5,22)*OM222+s.In(6,22)*OM322)-s.dpt(2,40)*(Fq223*S23+Fq323*C23));
 ...
 s.In(6,22)*OM222+s.In(9,22)*OM322)-OM322*(s.In(1,22)*OM122+s.In(2,22)*OM222));
  Cq322 = -(s.trq(3,22)-s.In(6,22)*OMp222-s.In(9,22)*OMp322-Cq223*S23-Cq323*C23+Fq123*s.dpt(2,40)+Fs122*s.l(2,22)-Fs222*s.l(1,22)-OM122*(...
 s.In(2,22)*OM122+s.In(5,22)*OM222+s.In(6,22)*OM322)+OM222*(s.In(1,22)*OM122+s.In(2,22)*OM222));
  ...
  Cq121 = -(s.trq(1,21)-s.In(1,21)*OMp121-Cq122*C22-Cq126*C26-Cq322*S22-Cq326*S26+Fq222*s.dpt(3,37)+Fq226*s.dpt(3,38)+Fs221*s.l(3,21)-Fs321*...
 s.l(2,21)-OM221*(s.In(6,21)*OM221+s.In(9,21)*OM321)+OM321*(s.In(5,21)*OM221+s.In(6,21)*OM321)+s.dpt(2,37)*(Fq122*S22-Fq322*C22)+s.dpt(2,38)*(Fq126*...
 S26-Fq326*C26));
 ...
 s.In(9,21)*OM321)-s.dpt(1,37)*(Fq122*S22-Fq322*C22)-s.dpt(1,38)*(Fq126*S26-Fq326*C26)-s.dpt(3,37)*(Fq122*C22+Fq322*S22)-s.dpt(3,38)*(Fq126*C26+Fq326*...
 S26));
  Cq321 = -(s.trq(3,21)-s.In(6,21)*OMp221-s.In(9,21)*OMp321+Cq122*S22+Cq126*S26-Cq322*C22-Cq326*C26-Fq222*s.dpt(1,37)-Fq226*s.dpt(1,38)+Fs121*...
 s.l(2,21)-Fs221*s.l(1,21)+OM121*(s.In(1,21)*OM221-s.In(5,21)*OM221-s.In(6,21)*OM321)+s.dpt(2,37)*(Fq122*C22+Fq322*S22)+s.dpt(2,38)*(Fq126*C26+Fq326*...
 S26));
 ...
 s.In(5,20)*OM320+s.In(9,20)*OM320)+s.dpt(3,33)*(Fq121*S21+Fq221*C21));
 ...
 s.In(1,20)*OM120+s.In(3,20)*OM320)-s.dpt(3,33)*(Fq121*C21-Fq221*S21));
 ...
 s.In(5,19)*OM219-s.In(6,19)*OM319));
  Fs118 = -(s.frc(1,18)-s.m(18)*(ALPHA117*C18-ALPHA317*S18-s.l(1,18)*(OM218*OM218+OM318*OM318)+s.l(3,18)*(BS318+OMp218)));
  Fs218 = -(s.frc(2,18)-s.m(18)*(ALPHA217+s.l(1,18)*(OMp318+OM118*OM218)-s.l(3,18)*(OMp118-OM218*OM318)));
  Fs318 = -(s.frc(3,18)-s.m(18)*(ALPHA117*S18+ALPHA317*C18+s.l(1,18)*(BS318-OMp218)-s.l(3,18)*(OM118*OM118+OM218*OM218)));
 ...
 s.In(9,16)*OM316)+s.dpt(3,24)*(Fq217*C17-Fq317*S17));
  Cq216 = -(s.trq(2,16)-s.In(5,16)*OMp216-Cq217*C17+Cq317*S17-Fq117*s.dpt(3,24)-Fs116*s.l(3,16)+Fs316*s.l(1,16)+OM116*(s.In(3,16)*OM116+...
 ...
 s.In(3,16)*OM316-s.In(5,16)*OM116));
  ...
  Fq215 = Fq216+Fs215;
  Cq115 = -(s.trq(1,15)-s.In(1,15)*OMp115-s.In(3,15)*OMp315-Cq116*C16-Cq316*S16+Fq216*s.dpt(3,22)+Fs215*s.l(3,15)-Fs315*s.l(2,15)-OM215*(...
 ...
 s.In(5,14)*OM214-s.In(9,14)*OM214)+s.dpt(3,20)*(Fq115*S15+Fq215*C15));
 ...
 s.In(2,14)*OM214-s.In(9,14)*OM114)-s.dpt(3,20)*(Fq115*C15-Fq215*S15));
 ...
 *(s.In(2,13)*OM113+s.In(5,13)*OM213+s.In(6,13)*OM313)-s.dpt(2,18)*(Fq214*S14+Fq314*C14));
 ...
 s.In(6,13)*OM213+s.In(9,13)*OM313)-OM313*(s.In(1,13)*OM113+s.In(2,13)*OM213));
  Cq313 = -(s.trq(3,13)-s.In(6,13)*OMp213-s.In(9,13)*OMp313-Cq214*S14-Cq314*C14+Fq114*s.dpt(2,18)+Fs113*s.l(2,13)-Fs213*s.l(1,13)-OM113*(...
 s.In(2,13)*OM113+s.In(5,13)*OM213+s.In(6,13)*OM313)+OM213*(s.In(1,13)*OM113+s.In(2,13)*OM213));
  Fs112 = -(s.frc(1,12)-s.m(12)*(ALPHA111*C12-ALPHA311*S12-s.l(1,12)*(OM212*OM212+OM312*OM312)+s.l(3,12)*(BS312+OMp212)));
  Fs212 = -(s.frc(2,12)-s.m(12)*(ALPHA211+s.l(1,12)*(OMp312+OM112*OM212)-s.l(3,12)*(OMp112-OM212*OM312)));
  Fs312 = -(s.frc(3,12)-s.m(12)*(ALPHA111*S12+ALPHA311*C12+s.l(1,12)*(BS312-OMp212)-s.l(3,12)*(OM112*OM112+OM212*OM212)));
 ...
 *(s.In(1,11)*OM111+s.In(2,11)*OM211+s.In(3,11)*OM311));
 ...
 OM38)-s.dpt(3,8)*(Fq19*C9-Fq29*S9));
  ...
  Cq17 = -(s.trq(1,7)-Cq18-s.In(1,7)*OMp17+Fs27*s.l(3,7)-Fs37*s.l(2,7)+OM27*OM37*(s.In(5,7)-s.In(9,7))-s.dpt(2,6)*(Fq28*S8+Fq38*C8));
  Cq27 = -(s.trq(2,7)-s.In(5,7)*OMp27-Cq28*C8+Cq38*S8-Fs17*s.l(3,7)+Fs37*s.l(1,7)-OM17*OM37*(s.In(1,7)-s.In(9,7)));
  Cq37 = -(s.trq(3,7)-s.In(9,7)*OMp37-Cq28*S8-Cq38*C8+Fq18*s.dpt(2,6)+Fs17*s.l(2,7)-Fs27*s.l(1,7)+OM17*OM27*(s.In(1,7)-s.In(5,7)));
 ...
 s.In(6,6)*OM26+s.In(9,6)*OM36)+OM36*(s.In(5,6)*OM26+s.In(6,6)*OM36)+s.dpt(2,1)*(Fq17*S7-Fq37*C7)+s.dpt(2,2)*(Fq113*S13-Fq313*C13)+s.dpt(3,3)*(Fq219*...
 C19-Fq319*S19));
  Cq26 = -(s.trq(2,6)-Cq213-Cq27-s.In(5,6)*OMp26-s.In(6,6)*OMp36-Cq219*C19+Cq319*S19-Fq119*s.dpt(3,3)-Fs16*s.l(3,6)+Fs36*s.l(1,6)+OM16*(s.In(3,6)...
 *OM16+s.In(6,6)*OM26+s.In(9,6)*OM36)-OM36*(s.In(1,6)*OM16+s.In(3,6)*OM36)+s.dpt(1,3)*(Fq219*S19+Fq319*C19));
 ...
 s.l(1,6)-OM16*(s.In(5,6)*OM26+s.In(6,6)*OM36)+OM26*(s.In(1,6)*OM16+s.In(3,6)*OM36)-s.dpt(1,3)*(Fq219*C19-Fq319*S19)+s.dpt(2,1)*(Fq17*C7+Fq37*S7)+...
