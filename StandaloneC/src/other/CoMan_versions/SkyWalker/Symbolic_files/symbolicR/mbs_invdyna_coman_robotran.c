...
// Forward Kinematics 
  ...
  ALPHA122 = C22*(ALPHA121+BETA221*s->dpt[2][39]+BETA321*s->dpt[3][39]+BS121*s->dpt[1][39])-S22*(ALPHA321+BETA721*
 s->dpt[1][39]+BETA821*s->dpt[2][39]+BS921*s->dpt[3][39]);
  ALPHA222 = ALPHA221+BETA421*s->dpt[1][39]+BETA621*s->dpt[3][39]+BS521*s->dpt[2][39];
  ALPHA322 = C22*(ALPHA321+BETA721*s->dpt[1][39]+BETA821*s->dpt[2][39]+BS921*s->dpt[3][39])+S22*(ALPHA121+BETA221*
 s->dpt[2][39]+BETA321*s->dpt[3][39]+BS121*s->dpt[1][39]);
  OM123 = qd[23]+OM122;
  ...
  ALPHA126 = C26*(ALPHA121+BETA221*s->dpt[2][40]+BETA321*s->dpt[3][40]+BS121*s->dpt[1][40])-S26*(ALPHA321+BETA721*
 s->dpt[1][40]+BETA821*s->dpt[2][40]+BS921*s->dpt[3][40]);
  ALPHA226 = ALPHA221+BETA421*s->dpt[1][40]+BETA621*s->dpt[3][40]+BS521*s->dpt[2][40];
  ALPHA326 = C26*(ALPHA321+BETA721*s->dpt[1][40]+BETA821*s->dpt[2][40]+BS921*s->dpt[3][40])+S26*(ALPHA121+BETA221*
 s->dpt[2][40]+BETA321*s->dpt[3][40]+BS121*s->dpt[1][40]);
  OM127 = qd[27]+OM126;
...
// Backward Dynamics 
  ...
  Cq233 = -(s->trq[2][33]-qdd[33]*s->In[5][33]-OM133*OM333*(s->In[1][33]-s->In[9][33])+s->l[3][33]*(s->frc[1][33]-
 s->m[33]*(s->g[3]*C32*S33+s->l[3][33]*(qdd[33]+OM133*OM333))));
  Cq132 = -(C33*(s->trq[1][33]+qd[33]*OM333*(s->In[5][33]-s->In[9][33])-s->In[1][33]*OMp133-s->l[3][33]*(s->frc[2][33]+
 s->m[33]*(s->g[3]*S32+s->l[3][33]*(OMp133-qd[33]*OM333))))+S33*(s->trq[3][33]+qd[33]*OM133*(s->In[1][33]-s->In[5][33])-
 s->In[9][33]*(qd[32]*qd[33]*C33+qdd[32]*S33)));
  Cq231 = -(s->trq[2][31]-qdd[31]*s->In[5][31]-OM131*OM331*(s->In[1][31]-s->In[9][31])+s->l[3][31]*(s->frc[1][31]-
 s->m[31]*(s->g[3]*C30*S31+s->l[3][31]*(qdd[31]+OM131*OM331))));
  Cq130 = -(C31*(s->trq[1][31]+qd[31]*OM331*(s->In[5][31]-s->In[9][31])-s->In[1][31]*OMp131-s->l[3][31]*(s->frc[2][31]+
 s->m[31]*(s->g[3]*S30+s->l[3][31]*(OMp131-qd[31]*OM331))))+S31*(s->trq[3][31]+qd[31]*OM131*(s->In[1][31]-s->In[5][31])-
 s->In[9][31]*(qd[30]*qd[31]*C31+qdd[30]*S31)));
  Fs129 = -(s->frc[1][29]+s->m[29]*(s->l[1][29]*(OM229*OM229+OM329*OM329)-s->l[2][29]*(BS229-OMp329)-s->l[3][29]*(BS329+
 OMp229)-C29*(ALPHA128+BETA328*s->dpt[3][56])+S29*(ALPHA328+BS928*s->dpt[3][56])));
  Fs229 = -(s->frc[2][29]-s->m[29]*(ALPHA228+BETA628*s->dpt[3][56]+s->l[1][29]*(BS229+OMp329)-s->l[2][29]*(OM129*OM129+
 ...
 *OM229)+C29*(ALPHA328+BS928*s->dpt[3][56])+S29*(ALPHA128+BETA328*s->dpt[3][56])));
 ...
 s->dpt[3][56]-Fs328*s->l[2][28]-OM228*(s->In[3][28]*OM128+s->In[6][28]*OM228+s->In[9][28]*OM328)+OM328*(s->In[5][28]*OM228+
 s->In[6][28]*OM328));
  Cq228 = -(s->trq[2][28]-Cq229-s->In[5][28]*OMp228-s->In[6][28]*OMp328-Fs128*s->l[3][28]+Fs328*s->l[1][28]+OM128*(
 s->In[3][28]*OM128+s->In[6][28]*OM228+s->In[9][28]*OM328)-OM328*(s->In[1][28]*OM128+s->In[3][28]*OM328)-s->dpt[3][56]*(Fs129
 *C29+Fs329*S29));
 ...
 s->In[5][27]*OM227+s->In[6][27]*OM327)+s->dpt[3][54]*(Fq128*S28+Fq228*C28));
 ...
 s->In[2][27]*OM227+s->In[3][27]*OM327)-s->dpt[3][54]*(Fq128*C28-Fq228*S28));
 ...
 s->In[9][26])-s->dpt[2][52]*(Fq227*S27+Fq327*C27));
  Cq226 = -(s->trq[2][26]-s->In[5][26]*OMp226-Cq227*C27+Cq327*S27-Fs126*s->l[3][26]+Fs326*s->l[1][26]-OM126*OM326*(
 s->In[1][26]-s->In[9][26]));
  Cq326 = -(s->trq[3][26]-s->In[9][26]*OMp326-Cq227*S27-Cq327*C27+Fq127*s->dpt[2][52]+Fs126*s->l[2][26]-Fs226*
 ...
 OMp225)-C25*(ALPHA124+BETA324*s->dpt[3][48])+S25*(ALPHA324+BS924*s->dpt[3][48])));
  Fs225 = -(s->frc[2][25]-s->m[25]*(ALPHA224+BETA624*s->dpt[3][48]+s->l[1][25]*(BS225+OMp325)-s->l[2][25]*(OM125*OM125+
 ...
 *OM225)+C25*(ALPHA324+BS924*s->dpt[3][48])+S25*(ALPHA124+BETA324*s->dpt[3][48])));
 ...
 s->l[3][24]+Fs225*s->dpt[3][48]-Fs324*s->l[2][24]-OM224*(s->In[3][24]*OM124+s->In[9][24]*OM324)+OM324*(s->In[2][24]*OM124+
 s->In[5][24]*OM224));
  Cq224 = -(s->trq[2][24]-Cq225-s->In[2][24]*OMp124-s->In[5][24]*OMp224-Fs124*s->l[3][24]+Fs324*s->l[1][24]+OM124*(
 s->In[3][24]*OM124+s->In[9][24]*OM324)-OM324*(s->In[1][24]*OM124+s->In[2][24]*OM224+s->In[3][24]*OM324)-s->dpt[3][48]*(Fs125
 *C25+Fs325*S25));
 ...
 s->l[2][23]-OM223*(s->In[3][23]*OM123-s->In[5][23]*OM323+s->In[9][23]*OM323)+s->dpt[3][46]*(Fq124*S24+Fq224*C24));
  Cq223 = -(s->trq[2][23]-s->In[5][23]*OMp223-Cq124*S24-Cq224*C24-Fs123*s->l[3][23]+Fs323*s->l[1][23]+OM123*(
 s->In[3][23]*OM123+s->In[9][23]*OM323)-OM323*(s->In[1][23]*OM123+s->In[3][23]*OM323)-s->dpt[3][46]*(Fq124*C24-Fq224*S24));
 ...
 s->In[6][22]*OM222+s->In[9][22]*OM322)+OM322*(s->In[2][22]*OM122+s->In[5][22]*OM222+s->In[6][22]*OM322)-s->dpt[2][44]*(Fq223
 *S23+Fq323*C23));
 ...
 s->l[3][22]+Fs322*s->l[1][22]+OM122*(s->In[6][22]*OM222+s->In[9][22]*OM322)-OM322*(s->In[1][22]*OM122+s->In[2][22]*OM222));
  Cq322 = -(s->trq[3][22]-s->In[6][22]*OMp222-s->In[9][22]*OMp322-Cq223*S23-Cq323*C23+Fq123*s->dpt[2][44]+Fs122*
 ...
 s->In[2][22]*OM222));
  ...
  Cq121 = -(s->trq[1][21]-s->In[1][21]*OMp121-Cq122*C22-Cq126*C26-Cq322*S22-Cq326*S26+Fq222*s->dpt[3][39]+Fq226*
 s->dpt[3][40]+Fs221*s->l[3][21]-Fs321*s->l[2][21]-OM221*(s->In[6][21]*OM221+s->In[9][21]*OM321)+OM321*(s->In[5][21]*OM221+
 s->In[6][21]*OM321)+s->dpt[2][39]*(Fq122*S22-Fq322*C22)+s->dpt[2][40]*(Fq126*S26-Fq326*C26));
  Cq221 = -(s->trq[2][21]-Cq222-Cq226-s->In[5][21]*OMp221-s->In[6][21]*OMp321-Fs121*s->l[3][21]+Fs321*s->l[1][21]-OM121*
 (s->In[1][21]*OM321-s->In[6][21]*OM221-s->In[9][21]*OM321)-s->dpt[1][39]*(Fq122*S22-Fq322*C22)-s->dpt[1][40]*(Fq126*S26-
 Fq326*C26)-s->dpt[3][39]*(Fq122*C22+Fq322*S22)-s->dpt[3][40]*(Fq126*C26+Fq326*S26));
  Cq321 = -(s->trq[3][21]-s->In[6][21]*OMp221-s->In[9][21]*OMp321+Cq122*S22+Cq126*S26-Cq322*C22-Cq326*C26-Fq222*
 s->dpt[1][39]-Fq226*s->dpt[1][40]+Fs121*s->l[2][21]-Fs221*s->l[1][21]+OM121*(s->In[1][21]*OM221-s->In[5][21]*OM221-
 s->In[6][21]*OM321)+s->dpt[2][39]*(Fq122*C22+Fq322*S22)+s->dpt[2][40]*(Fq126*C26+Fq326*S26));
 ...
 s->l[2][20]-OM220*(s->In[3][20]*OM120-s->In[5][20]*OM320+s->In[9][20]*OM320)+s->dpt[3][35]*(Fq121*S21+Fq221*C21));
  Cq220 = -(s->trq[2][20]-s->In[5][20]*OMp220-Cq121*S21-Cq221*C21-Fs120*s->l[3][20]+Fs320*s->l[1][20]+OM120*(
 s->In[3][20]*OM120+s->In[9][20]*OM320)-OM320*(s->In[1][20]*OM120+s->In[3][20]*OM320)-s->dpt[3][35]*(Fq121*C21-Fq221*S21));
 ...
 s->In[3][16]*OM116-s->In[5][16]*OM316+s->In[9][16]*OM316)+s->dpt[3][26]*(Fq217*C17-Fq317*S17));
  Cq216 = -(s->trq[2][16]-s->In[5][16]*OMp216-Cq217*C17+Cq317*S17-Fq117*s->dpt[3][26]-Fs116*s->l[3][16]+Fs316*
 ...
 OM315*OM315)));
  ...
  Fq215 = Fq216+Fs215;
  Cq115 = -(s->trq[1][15]-s->In[1][15]*OMp115-s->In[3][15]*OMp315-Cq116*C16-Cq316*S16+Fq216*s->dpt[3][24]+Fs215*
 ...
 s->In[9][15]*OM315)-OM315*(s->In[1][15]*OM115+s->In[3][15]*OM315)-s->dpt[3][24]*(Fq116*C16+Fq316*S16));
 ...
 s->l[2][14]+OM314*(s->In[2][14]*OM114+s->In[5][14]*OM214-s->In[9][14]*OM214)+s->dpt[3][22]*(Fq115*S15+Fq215*C15));
  Cq214 = -(s->trq[2][14]-s->In[2][14]*OMp114-s->In[5][14]*OMp214-Cq115*S15-Cq215*C15-Fs114*s->l[3][14]+Fs314*
 s->l[1][14]-OM314*(s->In[1][14]*OM114+s->In[2][14]*OM214-s->In[9][14]*OM114)-s->dpt[3][22]*(Fq115*C15-Fq215*S15));
 ...
 s->In[6][13]*OM213+s->In[9][13]*OM313)+OM313*(s->In[2][13]*OM113+s->In[5][13]*OM213+s->In[6][13]*OM313)-s->dpt[2][20]*(Fq214
 *S14+Fq314*C14));
 ...
 s->l[3][13]+Fs313*s->l[1][13]+OM113*(s->In[6][13]*OM213+s->In[9][13]*OM313)-OM313*(s->In[1][13]*OM113+s->In[2][13]*OM213));
  Cq313 = -(s->trq[3][13]-s->In[6][13]*OMp213-s->In[9][13]*OMp313-Cq214*S14-Cq314*C14+Fq114*s->dpt[2][20]+Fs113*
 ...
 s->In[2][10]*OM210+s->In[3][10]*OM310));
  ...
  Fq29 = Fq210+Fs29;
  Cq19 = -(s->trq[1][9]-s->In[1][9]*OMp19-s->In[2][9]*OMp29-s->In[3][9]*OMp39-Cq110*C10-Cq310*S10+Fq210*s->dpt[3][12]+
 ...
 *(s->In[3][9]*OM19+s->In[6][9]*OM29+s->In[9][9]*OM39)-OM39*(s->In[1][9]*OM19+s->In[2][9]*OM29+s->In[3][9]*OM39)-
 s->dpt[3][12]*(Fq110*C10+Fq310*S10));
 ...
 s->In[9][8]*OM38)+OM38*(s->In[5][8]*OM28+s->In[6][8]*OM38)+s->dpt[3][10]*(Fq19*S9+Fq29*C9));
  Cq28 = -(s->trq[2][8]-s->In[5][8]*OMp28-s->In[6][8]*OMp38-Cq19*S9-Cq29*C9-Fs18*s->l[3][8]+Fs38*s->l[1][8]-OM18*(
 s->In[1][8]*OM38-s->In[6][8]*OM28-s->In[9][8]*OM38)-s->dpt[3][10]*(Fq19*C9-Fq29*S9));
 ...
 s->dpt[2][8]*(Fq28*S8+Fq38*C8));
  Cq27 = -(s->trq[2][7]-s->In[5][7]*OMp27-Cq28*C8+Cq38*S8-Fs17*s->l[3][7]+Fs37*s->l[1][7]-OM17*OM37*(s->In[1][7]-
 s->In[9][7]));
  Cq37 = -(s->trq[3][7]-s->In[9][7]*OMp37-Cq28*S8-Cq38*C8+Fq18*s->dpt[2][8]+Fs17*s->l[2][7]-Fs27*s->l[1][7]+OM17*OM27*(
 s->In[1][7]-s->In[5][7]));
 ...
 s->dpt[2][3]*(Fq17*S7-Fq37*C7)+s->dpt[2][4]*(Fq113*S13-Fq313*C13)+s->dpt[3][5]*(Fq219*C19-Fq319*S19));
  Cq26 = -(s->trq[2][6]-Cq213-Cq27-s->In[5][6]*OMp26-s->In[6][6]*OMp36-Cq219*C19+Cq319*S19-Fq119*s->dpt[3][5]-Fs16*
 s->l[3][6]+Fs36*s->l[1][6]+OM16*(s->In[3][6]*OM16+s->In[6][6]*OM26+s->In[9][6]*OM36)-OM36*(s->In[1][6]*OM16+s->In[3][6]*OM36
 )+s->dpt[1][5]*(Fq219*S19+Fq319*C19));
 ...
 s->In[3][6]*OM36)-s->dpt[1][5]*(Fq219*C19-Fq319*S19)+s->dpt[2][3]*(Fq17*C7+Fq37*S7)+s->dpt[2][4]*(Fq113*C13+Fq313*S13));
