 ...
 *
 * modified by Nicolas Van der Noot
 *
 */
...
// modif: function added
double get_nstep_dopri5 (void)
{
  return nstep;
}
	      ...
	      double hmax, double* atoler, double* rtoler, int itoler, Loop_arguments *loop_arguments)
{
  ...
  for (i = 0; i < n; i++)
    yy1[i] = y[i] + h * f0[i];
  fcn (n, x+h, yy1, f1, loop_arguments); // modif: 'loop_arguments' added
...
static int dopcor (unsigned n, FcnEqDiff fcn, double x, double* y, double xend,
		   ...
		   double beta, double fac1, double fac2, unsigned* icont, Loop_arguments *loop_arguments) // modif: 'loop_arguments' added
{
  ...
  hlamb = 0.0;
  iasti = 0;
  fcn (n, x, y, k1, loop_arguments); // modif: 'loop_arguments' added
  hmax = fabs (hmax);
  iord = 5;
  if (h == 0.0)
    h = hinit (n, fcn, x, y, posneg, k1, k2, k3, iord, hmax, atoler, rtoler, itoler, loop_arguments); // modif: 'loop_arguments' added
  nfcn += 2;
  reject = 0;
  xold = x;
  if (iout)
  ...
  while (1)
  {
    ...
    // modif: stop the simulation if 'simu_go' is 0
    if (!loop_arguments->simu_go) 
    {
      xout = x;
      hout = h;
      return -4;
    ...
    fcn (n, x+c2*h, yy1, k2, loop_arguments); // modif: 'loop_arguments' added
    for (i = 0; i < n; i++)
      yy1[i] = y[i] + h * (a31*k1[i] + a32*k2[i]);
    fcn (n, x+c3*h, yy1, k3, loop_arguments); // modif: 'loop_arguments' added
    for (i = 0; i < n; i++)
      yy1[i] = y[i] + h * (a41*k1[i] + a42*k2[i] + a43*k3[i]);
    fcn (n, x+c4*h, yy1, k4, loop_arguments); // modif: 'loop_arguments' added
    for (i = 0; i <n; i++)
      yy1[i] = y[i] + h * (a51*k1[i] + a52*k2[i] + a53*k3[i] + a54*k4[i]);
    fcn (n, x+c5*h, yy1, k5, loop_arguments); // modif: 'loop_arguments' added
    for (i = 0; i < n; i++)
      ysti[i] = y[i] + h * (a61*k1[i] + a62*k2[i] + a63*k3[i] + a64*k4[i] + a65*k5[i]);
    xph = x + h;
    fcn (n, xph, ysti, k6, loop_arguments); // modif: 'loop_arguments' added
    for (i = 0; i < n; i++)
      yy1[i] = y[i] + h * (a71*k1[i] + a73*k3[i] + a74*k4[i] + a75*k5[i] + a76*k6[i]);
    fcn (n, xph, yy1, k2, loop_arguments); // modif: 'loop_arguments' added
    if (iout == 2)
    {
      if (nrds == n)
      {
...
int dopri5
 (unsigned n, FcnEqDiff fcn, double x, double* y, double xend, double* rtoler,
  ...
  long nmax, int meth, long nstiff, unsigned nrdens, unsigned* icont, unsigned licont, Loop_arguments *loop_arguments)
{
  int       arret, idid;
  unsigned  i;
  ...
  else
  {
    idid = dopcor (n, fcn, x, y, xend, hmax, h, rtoler, atoler, itoler, fileout,
		   solout, iout, nmax, uround, meth, nstiff, safe, beta, fac1, fac2, icont, loop_arguments); // modif: 'loop_arguments' added
    free (ysti);
    free (k6);
